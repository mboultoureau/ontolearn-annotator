...
class Builder(tfds.core.GeneratorBasedBuilder):
    ...
    def _split_generators(self, dl_manager: tfds.download.DownloadManager):
        ...
        path = Path("./datasets/active_dataset/data")
        ...
        return {
            ...
            "unlabelled": self._generate_examples(
                label_path=path / "unlabelled_labels.csv"
            ),
        }
    ...
    def _initalize_dataset(self):
        ...
        # Shuffle and separate in 5% train, 10% test, 10% validation and 75% unlabelled
        with open(path / "5k_epp_dataset.csv") as f:
            reader = csv.DictReader(f)
            rows = [row for row in reader]
            random.shuffle(rows)
            total = len(rows)
            train = int(total * 0.05)
            test = int(total * 0.1)
            validation = int(total * 0.1)
            unlabelled = total - train - test - validation
            print(f"Total: {total}, Train: {train}, Test: {test}, Validation: {validation}, Unlabelled: {unlabelled}")
            train_rows = rows[:train]
            test_rows = rows[train:train+test]
            validation_rows = rows[train+test:train+test+validation]
            unlabelled_rows = rows[train+test+validation:]
            ...
            with open(path / "unlabelled_labels.csv", "w") as f:
                writer = csv.DictWriter(f, fieldnames=reader.fieldnames)
                writer.writeheader()
                writer.writerows(unlabelled_rows)
        ...
        with label_path.open() as f:
          for row in csv.DictReader(f):
            ...
            yield image_id, {
                ...
                'image': f"datasets/active_dataset/data/images/{image_id}",
                'image_name': image_id
            }